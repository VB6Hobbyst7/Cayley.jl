...
28            Case "USD": Part1 = "US"
              StrangeFor18M = True
29            Case "ZAR": Part1 = "SA"
30            Case "CNH", "QAR", "RON": Throw "Swaption Vol Code not found"
31            Case Else: Throw "Unrecognised Ccy"
32        End Select

33        Select Case QuoteType
              Case "Normal": Part2 = "SN"
34            Case "OIS Normal"
35                If Ccy = "USD" Then
36                    Part2 = "SN"
37                ElseIf Ccy = "CHF" Then
38                    Part2 = "NO"
39                ElseIf Ccy = "DKK" Or Ccy = "SeK" Then
40                    Part2 = "NV"
41                Else
42                    Part2 = "Ne"
43                End If
44            Case Else: Throw "Unrecognised QuoteType. Allowed values are: Normal, OIS Normal"
45        End Select

46        Select Case UCase(exercise)
              Case "1M", "2M", "3M", "4M", "5M", "6M", "7M", "8M", "9M", "10M", "11M"
47                Part3 = "0" & Chr(64 + CLng(Left(exercise, Len(exercise) - 1)))  '1M = 0A, 2M = 0B etc
48            Case "13M", "14M", "15M", "16M", "17M", "18M"
                  If StrangeFor18M Then
                  Part3 = "0" & Chr(64 + CLng(Left(exercise, Len(exercise) - 1)))  '1M = 0A, 2M = 0B, 18M = 0R etc
                  Else
49                Part3 = "1" & Chr(64 - 12 + CLng(Left(exercise, Len(exercise) - 1)))    '13M = 1A, 14M = 1B etc
                  End If
50            Case "1Y", "2Y", "3Y", "4Y", "5Y", "6Y", "7Y", "8Y", "9Y", "10Y", "15Y", "20Y", "25Y", "30Y"
51                Part3 = Format(CLng(Left(exercise, Len(exercise) - 1)), NumberFormatexercise)
52            Case Else
53                Throw "Unrecognised exercise. Allowed values are: 1M, 2M, 3M,...18M, 1Y, 2Y, 3Y, 4Y, 5Y, 6Y, 7Y, 8Y, 9Y, 10Y, 15Y, 20Y, 25Y, 30Y"
54        End Select

55        Select Case UCase(Tenor)
              Case "1Y", "2Y", "3Y", "4Y", "5Y", "6Y", "7Y", "8Y", "9Y", "10Y", "15Y", "20Y", "25Y", "30Y"
56                Part4 = Format(CLng(Left(Tenor, Len(Tenor) - 1)), NumberFormatTenor)
57            Case Else
58                Throw "Unrecognised Tenor. Allowed values are: 1Y, 2Y, 3Y, 4Y, 5Y, 6Y, 7Y, 8Y, 9Y, 10Y, 15Y, 20Y, 25Y, 30Y"
59        End Select

60        Select Case Contributor
              Case "CMPN", "LAST", "BBIR", "CFIR", "CNTR", "ICPL", "SMKO", "TRPU", "GFIS", "CMPL"
61                Part5 = " " & Contributor
62            Case Else
63                Throw "Unrecognised Contributor. Allowed values are: CMPN, LAST, BBIR, CFIR, CNTR, ICPL, SMKO, TRPU, GFIS, CMPL"
64        End Select

65        BloombergTickerSwaptionVol = Part1 & Part2 & Part3 & Part4 & Part5 & Part6

66        Exit Function
ErrHandler:
67        BloombergTickerSwaptionVol = "#BloombergTickerSwaptionVol (line " & CStr(Erl) & "): " & Err.Description & "!"
End Function
