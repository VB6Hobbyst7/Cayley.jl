...
Sub ApplyComments()
          ...
          Dim TargetCell As Range, NewComment As Variant, OldComment As String, Prompt As String
...
22                On Error GoTo ErrHandler
                  Dim ReallyDifferent As Boolean, MsgBoxResult As VbMsgBoxResult
...
28                        Prompt = "Change comment at " + TargetCell.Parent.Name + "!" + Replace(TargetCell.Address, "$", "") + ": '" + CStr(TargetCell.Value) + "'?" + vbLf + vbLf + _
                          ...
                          Dim CheckBoxValue As Boolean, DoTheSame As Variant
...
Sub FixCellComments(ws As Worksheet)
          Dim SPH As clsSheetProtectionHandler, SUH As clsScreenUpdateHandler
          Dim R As Range, C As Range, cmt As Comment
